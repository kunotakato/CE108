...
学生がスマートフォンで毎日5問を解くための Next.js/TypeScript フロントエンドです。v0.4.4ではノートAIのファイル読み込みをFastAPI経由にし、写真・PDF OCRを安全に接続するための入口を整備しています。教員・管理者画面は従来どおり Streamlit を利用します。
...
写真・PDFのOCRは、FastAPI側で`NOTE_OCR_PROVIDER=disabled`の場合は外部AIへ送信されません。まずは`.txt`、`.md`、`.csv`の学習メモで検証してください。
...
- ノートAIでは`.txt`、`.md`、`.csv`の学習メモをFastAPI経由で読み込めます。写真・PDFはOCR接続設計済みですが、外部βでは既定で未有効です。
