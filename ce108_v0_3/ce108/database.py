...
CREATE TABLE IF NOT EXISTS past_exam_theme_refs(id INTEGER PRIMARY KEY AUTOINCREMENT,exam_round INTEGER NOT NULL,exam_year INTEGER,session TEXT NOT NULL CHECK(session IN ('午前','午後','AM','PM')),question_number INTEGER NOT NULL,topic_id INTEGER NOT NULL,derived_theme TEXT NOT NULL,keywords TEXT NOT NULL DEFAULT '[]',source_url TEXT,copyright_status TEXT NOT NULL DEFAULT 'metadata_only',official_text_included INTEGER NOT NULL DEFAULT 0 CHECK(official_text_included IN (0,1)),linked_question_id INTEGER,note TEXT,created_at TEXT NOT NULL,updated_at TEXT NOT NULL,UNIQUE(exam_round,session,question_number,topic_id,derived_theme),FOREIGN KEY(topic_id) REFERENCES topics(id),FOREIGN KEY(linked_question_id) REFERENCES questions(id));
...
CREATE TABLE IF NOT EXISTS student_notes(id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER NOT NULL,title TEXT NOT NULL,content TEXT NOT NULL,source_type TEXT NOT NULL DEFAULT 'manual_note',created_at TEXT NOT NULL,updated_at TEXT NOT NULL,FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE);
CREATE TABLE IF NOT EXISTS note_generated_questions(id INTEGER PRIMARY KEY AUTOINCREMENT,note_id INTEGER NOT NULL,user_id INTEGER NOT NULL,question_type TEXT NOT NULL DEFAULT 'single',question_text TEXT NOT NULL,choices TEXT NOT NULL,correct_code TEXT NOT NULL DEFAULT '1',choice_explanations TEXT NOT NULL,explanation TEXT NOT NULL,topic_code TEXT NOT NULL,status TEXT NOT NULL DEFAULT 'active',created_at TEXT NOT NULL,FOREIGN KEY(note_id) REFERENCES student_notes(id) ON DELETE CASCADE,FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE);
CREATE TABLE IF NOT EXISTS note_question_answers(id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER NOT NULL,note_question_id INTEGER NOT NULL,selected_code TEXT NOT NULL,is_correct INTEGER NOT NULL,confidence_level TEXT NOT NULL,response_time_seconds INTEGER NOT NULL DEFAULT 0,answered_at TEXT NOT NULL,FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE,FOREIGN KEY(note_question_id) REFERENCES note_generated_questions(id) ON DELETE CASCADE);
...
CREATE TABLE IF NOT EXISTS user_question_bookmarks(id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER NOT NULL,question_id INTEGER NOT NULL,note TEXT,created_at TEXT NOT NULL,UNIQUE(user_id,question_id),FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE,FOREIGN KEY(question_id) REFERENCES questions(id) ON DELETE CASCADE);
...
CREATE INDEX IF NOT EXISTS idx_bookmarks_user ON user_question_bookmarks(user_id,created_at);
CREATE INDEX IF NOT EXISTS idx_past_exam_refs_round_session ON past_exam_theme_refs(exam_round,session,question_number);
CREATE INDEX IF NOT EXISTS idx_past_exam_refs_topic ON past_exam_theme_refs(topic_id,exam_round);
'''
